...
(machineIdentifier.bin). Do NOT run more than one at the same time.
EOF
  exit 1
}
...
[[ -f "${TMPL}/machineIdentifier.bin" ]] || die "Template missing machineIdentifier.bin: ${TMPL}"
...
copy_file "${TMPL}/machineIdentifier.bin"       "${DST}/machineIdentifier.bin"
...
echo ""
echo "  [!] Same ECID as ${TMPL_NAME} — do NOT run multiple instances simultaneously."
